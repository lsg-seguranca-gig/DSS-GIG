...
(function(){
  // ===== Loader dinâmico com fallback LOCAL -> CDN =====
  ...
  function tryLoadPair(localSrc, cdnSrc){ return loadScript(localSrc).catch(function(){ return loadScript(cdnSrc); }); }
  function ensureLibs(){
    return Promise.resolve()
      .then(function(){ return tryLoadPair('/vendor/jspdf.umd.min.js','https://cdn.jsdelivr.net/npm/jspdf@2.5.1/dist/jspdf.umd.min.js'); })
      .then(function(){ return tryLoadPair('/vendor/jspdf.plugin.autotable.min.js','https://cdn.jsdelivr.net/npm/jspdf-autotable@3.8.2/dist/jspdf.plugin.autotable.min.js'); })
      .then(function(){ return tryLoadPair('/vendor/xlsx.full.min.js','https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js'); })
      .then(function(){ return tryLoadPair('/vendor/jszip.min.js','https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js'); })
      .then(function(){ return tryLoadPair('/vendor/FileSaver.min.js','https://cdn.jsdelivr.net/npm/file-saver@2.0.5/dist/FileSaver.min.js'); });
  }
  ...
  function parseTimestamp(v){
    ...
    var m = String(v).trim().match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?$/);
    if (m){
    ...
    return new Promise(function(resolve){
      ...
      img.onload = function(){
        try {
          ...
          window.LOGO_DATAURL=c.toDataURL('image/png');
          resolve(window.LOGO_DATAURL);
        } catch(e){ resolve(window.LOGO_DATAURL=''); }
      };
  ...
  function popularSemanas(){
    ...
    fetch('/api/gas?action=registros')
      ...
      .then(function(data){
        ...
        var tit = Object.keys(set).sort(function(a,b){ return a.localeCompare(b,'pt-BR',{sensitivity:'base'}); });
  ...
  function buscar(){
    ...
    var fDataI=document.getElementById('fDataInicio').value; var fDataF=document.getElementById('fDataFinal').value;
    ...
    fetch('/api/gas?'+qs.toString()).then(function(res){ return res.json(); }).then(function(data){
      ...
      var di=normalizarDataInput(fDataI); var df=fimDoDia(normalizarDataInput(fDataF));
  ...
  function renderTabela(list){
    ...
    for (var i=0;i<list.length;i++){
      var r=list[i]; var dataPart=formatTimestamp(r.Timestamp);
  ...
  // ===== XLS =====
  function gerarXLS(){
    window.ensureLibs().then(function(){
      ...
      var wb=XLSX.utils.book_new(); var ws=XLSX.utils.json_to_sheet(linhas);
    ...
    window.ensureLibs().then(function(){
      loadLogoAsDataURL().then(function(){
        ...
        fetchTreinamentos().then(function(tList){
          var titulos=[], seen=Object.create(null);
          ...
          var semanasISO=[], sseen=Object.create(null);
          ...
          if(tList && tList.length){
            ...
            if(!hit && titulos.length){ for (var y=0;y<tList.length;y++){ var tt2=tList[y]; if(String(tt2['Titulo']).trim()===titulos[0]){ hit=tt2; break; } } }
          ...
          for (var pi=1; pi<=total; pi++){
            ...
            var FOOT_Y=pageHeight-22; var label='Página '+pi+' de '+total;
    ...
    window.ensureLibs().then(function(){
      ...
      fetch('/api/gas?action=treinamentos').then(function(r){return r.json();}).then(function(tData){
        ...
        Promise.all([
          ...
          // HTML Offline - NENHUMA string possui quebra de linha literal
          var offlineHTML = ''
            + '<!DOCTYPE html>\n'
  ...
  var ids=['fMat','fNome','fDataInicio','fDataFinal','fSemanaTitulo'];
  for (var ii=0; ii<ids.length; ii++){
    var el=document.getElementById(ids[ii]);
    if(el){ el.addEventListener('keydown', function(e){ if(e.key==='Enter'){ e.preventDefault(); buscar(); } }); }
  }
